...
export const WysiwygImageBlock: React.FC<Props> = ({ block, isActive, onSelect, onUpdate }) => {
  ...
  const handleUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      img.onload = () => {
        ...
        const displayW = Math.min(140, img.naturalWidth * 0.264);
        onUpdate({
          src: urlData.publicUrl,
          originalWidth: img.naturalWidth,
          originalHeight: img.naturalHeight,
          displayWidth: Math.round(displayW),
          displayHeight: Math.round(displayW / aspect),
        });
        setUploading(false);
      };
      img.onerror = () => { onUpdate({ src: urlData.publicUrl }); setUploading(false); };
  ...
  return (
    <div
      className={`relative group transition-shadow ${
        isActive ? 'ring-2 ring-primary/60 ring-offset-1 rounded-sm' : 'hover:ring-1 hover:ring-muted-foreground/20 rounded-sm'
      }`}
      style={{
    ...
    >
      <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => { if (e.target.files?.[0]) handleUpload(e.target.files[0]); e.target.value = ''; }} />
      ...
      {uploading ? (
        ...
        <div className="inline-block relative">
          ...
          />
          {block.caption && <p className="text-xs text-muted-foreground italic text-center mt-1">{block.caption}</p>}
          <button
