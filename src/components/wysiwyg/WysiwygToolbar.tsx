...
import {
  Type, Image, Table2, Minus, Plus, FileDown, FileText, Save,
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Separator } from '@/components/ui/separator';
import { Tooltip, TooltipContent, TooltipTrigger } from '@/components/ui/tooltip';
import {
  createDefaultTextBlock, createDefaultImageBlock, createDefaultSpacerBlock,
...
export const WysiwygToolbar: React.FC<Props> = ({
  onAddBlock, onAddPage, onExportPdf, onExportDocx, isDirty, isSaving, title,
}) => {
  ...
  const addTable = () => {
    const block: TableBlock = {
      ...
      marginTop: 4, marginBottom: 4, padding: 0, alignment: 'left', width: 100,
      rows: 3, cols: 3,
      data: Array.from({ length: 3 }, () => Array.from({ length: 3 }, () => '')),
      colWidths: [33.3, 33.3, 33.4], headerRow: true, borderColor: '#000000', borderWidth: 1,
    };
    onAddBlock(block);
  };
  ...
  return (
    <div className="flex items-center gap-1 px-4 py-2 border-b bg-background sticky top-0 z-20">
      <div className="flex items-center gap-2 mr-4">
        ...
        {isDirty && !isSaving && <span className="text-xs text-muted-foreground">• Não salvo</span>}
      </div>
      ...
      <div className="flex items-center gap-0.5 mx-2">
        {[
          { icon: Type, label: 'Texto', fn: addText },
          { icon: Image, label: 'Imagem', fn: addImage },
          { icon: Table2, label: 'Tabela', fn: addTable },
          { icon: Minus, label: 'Espaçador', fn: addSpacer },
        ].map(({ icon: Icon, label, fn }) => (
          <Tooltip key={label}>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={fn}>
